...
from .build.inference import PointProcessCondLaw
    ...
    * :math:`f_{ij}` the mark functions supposed to be piece-wise constant
    ...
    def incremental_fit(self, realization, T=None, compute=True):
        ...
        if self.n_realizations == 0:
            ...
            warnings.warn(("compute() method was already called, computed kernels will be updated."))
        ...
        if last_event_time < 0:
            warnings.warn("An empty realization was passed. No computation was performed.")
            return
        ...
        if T is None:
            ...
            raise ValueError("Argument T (%g) specified is too small, "
                             "you should use default value or a value greater or equal to %g." \
                              % (T, last_event_time))
...
#                raise ValueError(msg)
    ...
    def _PointProcessCondLaw(self, realization, index, i, j, l, T):
        ...
        # TODO: this lock acquire is very expensive here  --> Do we really need it ??
        if self.n_threads > 1:
            self._lock.acquire()
    ...
    def compute(self):
        ...
        if any(self._n_events[0,:]==0):
            ...
            msg = "Cannot run estimation : not enough events for components %s" % str(k)
            raise ValueError(msg)
