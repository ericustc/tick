...
void PointProcessCondLaw(ArrayDouble &y_time,
    ...
    if (z_time.size() != z_mark.size()) {
        TICK_ERROR("z_time (size=" << z_time.size() << ") and z_mark (size=" << z_mark.size()<< ") should have the same size");
    }

    if (res_X.size() != res_Y.size()) {
        TICK_ERROR("res_X (size=" << res_X.size() << ") and res_Y (size=" << res_Y.size()<< ") should have the same size");
    }

    if (res_X.size() != lags.size()-1) {
        TICK_ERROR("lags (size=" << lags.size() << ") should be of the size of res_X (size=" << res_X.size()<< " plus one");
    }
